...
//记住最后indexpath的放置，在滚动时以不一样的indexpaths重新布局
...
- (void) initialize {
    ...
    self.blockPixels = CGSizeMake(1, 1);
}
...
- (UICollectionViewLayoutAttributes *)layoutAttributesForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    CGRect frame = [self frameForIndexPath:indexPath];//frame.sise=(x,y)//x和y的值分别是75、150、225
...
- (void) prepareLayout {
    ...
    //现在的[self blockPixels].height=[self blockPixels].width =75 =375/5
    else
...
// returning no in the callback will
// terminate the iterations early 回调的时候返回no将会很早的终止迭代器
//
...
//很重要，为每一个cell设置frame，注意需要乘blockPixels
- (CGRect) frameForIndexPath:(NSIndexPath*)path {
    ...
    CGRect contentRect = UIEdgeInsetsInsetRect(self.collectionView.frame, self.collectionView.contentInset);
    if (isVert) {
        float initialPaddingForContraintedDimension = (CGRectGetWidth(contentRect) - [self restrictedDimensionBlockSize])/ 2;
...
//This method is prefixed（前缀） with get because it may return its value indirectly（间接地）
